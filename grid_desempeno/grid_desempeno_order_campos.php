<?php
   ...
   if (!function_exists("NM_is_utf8"))
   {
    ...
    $Ord_Cmp = new grid_desempeno_Ord_cmp(); 
    $Ord_Cmp->Ord_cmp_init();
   
class grid_desempeno_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $tab_ger_campos, $tab_def_campos, $tab_converte, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   $tab_labels["codigo"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_desempeno']['labels']["codigo"])) ? $_SESSION['sc_session'][$sc_init]['grid_desempeno']['labels']["codigo"] : "Codigo";
   $tab_ger_campos['periodos'] = "on";
   $tab_def_campos['periodos'] = "periodos";
   $tab_converte["periodos"]   = "periodos";
   $tab_labels["periodos"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_desempeno']['labels']["periodos"])) ? $_SESSION['sc_session'][$sc_init]['grid_desempeno']['labels']["periodos"] : "Periodos";
   $tab_ger_campos['superior'] = "on";
   $tab_def_campos['superior'] = "superior";
   $tab_converte["superior"]   = "superior";
   $tab_labels["superior"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_desempeno']['labels']["superior"])) ? $_SESSION['sc_session'][$sc_init]['grid_desempeno']['labels']["superior"] : "Indicadores de Desempeño";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_desempeno']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_desempeno']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_desempeno']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_desempeno']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_desempeno']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_desempeno']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_desempeno']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_desempeno']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       {
           $_SESSION['sc_session'][$sc_init]['grid_desempeno']['ordem_select'][$campo] = $ordem;
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   $.ajax({
    type: "POST",
    url: "grid_desempeno_order_campos.php",
    data: {
...
<?php
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if (!isset($_SESSION['sc_session'][$sc_init]['grid_desempeno']['ordem_select'][$tab_def_campos[$NM_cada_field]]))
           {
?>
...
<?php
   foreach ($_SESSION['sc_session'][$sc_init]['grid_desempeno']['ordem_select'] as $NM_cada_field => $NM_cada_opc)
   {
