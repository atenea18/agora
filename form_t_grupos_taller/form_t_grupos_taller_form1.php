<div id="form_t_grupos_taller_form1" style='display: none; width: 1px; height: 0px; overflow: scroll'>
...
<?php
 ...
 $sDetailSrc = ('novo' == $this->nmgp_opcao) ? 'form_t_grupos_taller_empty.htm' : $this->Ini->link_form_grupo_x_asig_x_doce_edit . '?script_case_init=' . $this->form_encode_input($this->Ini->sc_page . '&script_case_session=' . session_id() . '&script_case_detail=Y&sc_ifr_height=600');
?>
...
<?php if (('novo' != $this->nmgp_opcao || $this->Embutida_form) && !$this->nmgp_form_empty && $_SESSION['sc_session'][$this->Ini->sc_page]['form_t_grupos_taller']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_t_grupos_taller']['run_iframe'] != "F") { if ('parcial' == $this->form_paginacao) {?><script>summary_atualiza(<?php echo ($_SESSION['sc_session'][$this->Ini->sc_page]['form_t_grupos_taller']['reg_start'] + 1). ", " . $_SESSION['sc_session'][$this->Ini->sc_page]['form_t_grupos_taller']['reg_qtd'] . ", " . ($_SESSION['sc_session'][$this->Ini->sc_page]['form_t_grupos_taller']['total'] + 1)?>);</script><?php }} ?>
<?php if (('novo' != $this->nmgp_opcao || $this->Embutida_form) && !$this->nmgp_form_empty && $_SESSION['sc_session'][$this->Ini->sc_page]['form_t_grupos_taller']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_t_grupos_taller']['run_iframe'] != "F") { if ('total' == $this->form_paginacao) {?><script>summary_atualiza(1, <?php echo $this->sc_max_reg . ", " . $this->sc_max_reg?>);</script><?php }} ?>
<?php if (('novo' != $this->nmgp_opcao || $this->Embutida_form) && !$this->nmgp_form_empty && $_SESSION['sc_session'][$this->Ini->sc_page]['form_t_grupos_taller']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_t_grupos_taller']['run_iframe'] != "F") { ?><script>navpage_atualiza('<?php echo $this->SC_nav_page ?>');</script><?php } ?>
</td></tr> 
</table> 
</div> 
</td> 
...
<script> 
<?php
 $NM_pag_atual = "form_t_grupos_taller_form0";
 if (isset($this->nmgp_ancora) && $this->nmgp_ancora != "")
 {
     $NM_pag_atual = "form_t_grupos_taller_form" . $this->nmgp_ancora;
 }
?>
...
<script>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_t_grupos_taller']['masterValue']))
{
?>
...
?>
<script>
 parent.scAjaxDetailStatus("form_t_grupos_taller");
 parent.scAjaxDetailHeight("form_t_grupos_taller", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
}
