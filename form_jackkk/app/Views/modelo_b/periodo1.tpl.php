...
<table class="table content-table " id="content-inputs">
	...
	<thead class="">
		<tr id="background-rows-table"  valign="middle">
			...
			<th colspan="6"> <?php echo $porcentajes['etiqueta_grupo_1']." ".($porcentajes['porcentaje_efp']+$porcentajes['porcentaje_grupo1']); ?> 		
			
			
			</th>
			<th rowspan="2"><?php echo ($porcentajes['porcentaje_efp']+$porcentajes['porcentaje_grupo1']); ?>%</th>
			...
			<th colspan="5" ><?=$porcentajes['etiqueta_grupo_2']; ?></th>			
			<th rowspan="2"><?=$porcentajes['porcentaje_grupo2']?>%</th>

			<th colspan="5" ><?=$porcentajes['etiqueta_grupo_3']; ?></th>			
		...
		<tr id="item-posicion" valign="middle" class="border-th">
			...
			<th data-update="dc1" data-estado="false">
				<?php //foreach ($codigos  as  $row) 	{ echo $row['posicion']=='dc1'?$row["cod_desemp"]:'';}?>				
			</th>

			<th data-update="dc2" data-estado="false">
				
			</th>
			<th data-update="dc3" data-estado="false">
				
			</th>
			<th data-update="dc4" data-estado="false">
				
			</th>
			<th data-update="dc5" data-estado="false">
				
			</th>
			<th >EFP <?=$porcentajes['porcentaje_efp']; ?>%</th>
	...
	foreach ($datos  as $clave => $row) {
		...
		<tr class="<?=$active = $num%2==0?'active':''?> inputs" id="<?=$row['id_estudiante']?>" >
			...
			<td>
				<input data-id="<?=$row['id_estudiante']?>" data-desemp="dt" name="dc6" data-cont="<?=$cont++;?>" step="0.1"   type="text"   class="form-control"   value="<?=$row['dc6']?>"> 

					<input data-id="<?=$row['id_estudiante']?>" data-desemp="grupodt" data-grupo="lista" name="dc7" type="hidden" class="form-control input-sin-borde" readonly disabled   value="<?=$row['dc7']?>">					              
			</td>
			<td>  
			...
			<td>
				<input data-id="<?=$row['id_estudiante']?>" data-desemp="dp" name="dp4" data-cont="<?=$cont++;?>" step="0.1"   type="text"  class="form-control"   value="<?=$row['dp4']?>">                
			</td>
			<td>
				<input data-id="<?=$row['id_estudiante']?>" data-desemp="dp" name="dp5" data-cont="<?=$cont++;?>" step="0.1"   type="text"  class="form-control"   value="<?=$row['dp5']?>">                
			</td>
			<td>
			...
			<td>
				<input data-id="<?=$row['id_estudiante']?>" data-desemp="ds" name="ds4" data-cont="<?=$cont++;?>" step="0.1"   type="text"  class="form-control" value="<?=$row['ds4']?>">                 
			</td>
			<td>
				<input data-id="<?=$row['id_estudiante']?>" data-desemp="ds" name="ds5" data-cont="<?=$cont++;?>" step="0.1"   type="text"  class="form-control" value="<?=$row['ds5']?>">                 
			</td>
			<td>
