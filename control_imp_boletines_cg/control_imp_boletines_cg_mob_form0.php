...
?>
 ...
 <?php
 if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg']['embutida_pdf']))
 {
 ?>
...
?>
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>control_imp_boletines_cg/control_imp_boletines_cg_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
...
<?php
include_once("control_imp_boletines_cg_mob_sajax_js.php");
?>
<script type="text/javascript">
...
include_once('control_imp_boletines_cg_mob_jquery.php');
 ...
 function addAutocomplete(elem) {
  ...
  $(".sc-ui-autocomp-id_estudent", elem).focus(function() {
   ...
   source: function (request, response) {
    $.ajax({
     url: "control_imp_boletines_cg_mob.php",
     dataType: "json",
     data: {
      term: request.term,
...
<?php
$str_iframe_body = ('F' == $_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg_mob']['run_iframe'] || 'R' == $_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg_mob']['run_iframe']) ? 'margin: 2px;' : '';
...
if ($GLOBALS["erro_incl"] == 1)
{
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg_mob']['opc_ant'] = "novo";
    $_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg_mob']['recarga'] = "novo";
}
if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg_mob']['recarga']))
{
    $opcao_botoes = $this->nmgp_opcao;
}
else
{
    $opcao_botoes = $_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg_mob']['recarga'];
}
?>
...
<?php
 include_once("control_imp_boletines_cg_mob_js0.php");
?>
<script type="text/javascript"> 
...
<form name="F1" method="post" 
               action="control_imp_boletines_cg_mob.php" 
               target="_self"> 
...
<?php
$_SESSION['scriptcase']['error_span_title']['control_imp_boletines_cg_mob'] = $this->Ini->Error_icon_span;
$_SESSION['scriptcase']['error_icon_title']['control_imp_boletines_cg_mob'] = '' != $this->Ini->Err_ico_title ? $this->Ini->path_icones . '/' . $this->Ini->Err_ico_title : '';
?>
...
<?php
  if (!$this->Embutida_call && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg_mob']['mostra_cab']) || $_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg_mob']['mostra_cab'] != "N"))
  {
?>
<tr><td>
<style>
...
<?php
  ...
  {
       if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg']['where_filter']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg_mob']['empty_filter'] = true;
       }
  }
  else
  {
...
<?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
    if (!isset($this->nm_new_label['print_area']))
    {
        $this->nm_new_label['print_area'] = "Imprime Área";
    }
?>
<?php
   $nm_cor_fun_cel  = ($nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
   ...
   $sStyleHidden_print_area = '';
   if (isset($this->nmgp_cmp_hidden['print_area']) && $this->nmgp_cmp_hidden['print_area'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['print_area']);
       $sStyleHidden_print_area = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_print_area = 'display: none;';
   $sStyleReadInp_print_area = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['print_area']) && $this->nmgp_cmp_readonly['print_area'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['print_area']);
       $sStyleReadLab_print_area = '';
       $sStyleReadInp_print_area = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['print_area']) && $this->nmgp_cmp_hidden['print_area'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="print_area" value="<?php echo $this->form_encode_input($print_area) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOdd css_print_area_line" id="hidden_field_data_print_area" style="<?php echo $sStyleHidden_print_area; ?>"> <span class="scFormLabelOddFormat css_print_area_label"><span id="id_label_print_area"><?php echo $this->nm_new_label['print_area']; ?></span></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["print_area"]) &&  $this->nmgp_cmp_readonly["print_area"] == "on") { 

 if ("1" == $this->print_area) { $print_area_look = "Si";} 
 if ("2" == $this->print_area) { $print_area_look = "No";} 
?>
<input type="hidden" name="print_area" value="<?php echo $this->form_encode_input($print_area) . "\">" . $print_area_look . ""; ?>
<?php } else { ?>
...
<?php
 ...
 if ("2" == $this->print_area) { $print_area_look = "No";} 
?>
<span id="id_read_on_print_area"  class="css_print_area_line" style="<?php echo $sStyleReadLab_print_area; ?>"><?php echo $this->form_encode_input($print_area_look); ?></span><span id="id_read_off_print_area" style="<?php echo $sStyleReadInp_print_area; ?>"><div id="idAjaxRadio_print_area" style="display: inline-block">
<TABLE cellspacing="0" cellpadding="0" border="0"><TR>
  <TD class="scFormDataFontOdd css_print_area_line">    <input style="float:left; position:relative; top: -3px;" type=radio name="print_area" value="1"
<?php $_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg_mob']['Lookup_print_area'][] = '1'; ?>
<?php  if ("1" == $this->print_area)  { echo " checked" ;} ?><?php  if (empty($this->print_area)) { echo " checked" ;} ?>  onClick="" >Si</TD>
  <TD class="scFormDataFontOdd css_print_area_line">    <input style="float:left; position:relative; top: -3px;" type=radio name="print_area" value="2"
<?php $_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg_mob']['Lookup_print_area'][] = '2'; ?>
<?php  if ("2" == $this->print_area)  { echo " checked" ;} ?>  onClick="" >No</TD>
</TR></TABLE>
</div>
</span><?php  }?>
 </TD>
...
<script>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_imp_boletines_cg_mob']['masterValue']))
{
?>
...
?>
<script>
 parent.scAjaxDetailStatus("control_imp_boletines_cg_mob");
 parent.scAjaxDetailHeight("control_imp_boletines_cg_mob", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
}
