...
<table class="table content-table " id="content-inputs">
    ...
    <tr id="background-rows-table"  valign="middle">
        ...
        <th colspan="5"> <?php echo $porcentajes['etiqueta_grupo_1']." ".$porcentajes['porcentaje_grupo1']; ?> %
    ...
    foreach ($datos  as $clave => $row) {
        ...
        <tr class="<?=$active = $num%2==0?'active':''?> inputs editable" id="<?=$row['id_estudiante']?>" >
            ...
            <td class="<?=($status?"hidden":"")?>">
                <?php
                if (!$status){ ?>
                    <input data-id="<?=$row['id_estudiante']?>" data-desemp="da" name="aeep_<?=$p;?>" data-cont="<?=
                    ($status?"":$cont++)?>" step="0.1"   type="text"  class="form-control" value="<?=$row['aeep_'.$p]?>">
                <?php } ?>
            </td>
            <td class="<?=($status?"hidden":"")?>">
                <?php
