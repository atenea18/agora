...
$(function()
{
	...
	var numMinimo = parseFloat($('#minimo').val());
			...
			$('#content-btn-criterios').find('input[data-criterio='+idDesemp+']').each(
				function(i,element){
					$(element).keyup(function(ev){
						...
						$.ajax({
							method: "POST",
							url: urls+"/Evaluacion/setUpdateCriterio/"+pos+"/"+databaseDB,
							data: obj
						})
						.done(function( msg ) {
	...
	$(document).on('ready',function(){       
		...
		.keyup(function(ev){
			...
			var valueElement = element.value;                              
				...
				stringCadena += element.name + ":'" + valueElement + "',";
			});
                ...
                if(stringCadena != ''){
                    ...
                    $.ajax({
                        ...
                        .done(function( msg ) {							
							...
							if(msg == 'false'){
								alert("Usuario no autorizado para realizar modificaciones");
							}
                        });
                }
...
function posicionesClick(ev){
    ...
    $.ajax({
        method: "POST",
        url: urls+"/Indicadores/deleteCodigosDesemp/"+posDelet+"/"+desemp+"/"+grup+"/"+asign+"/"+per+"/"+databaseDB,
        data: ''
    })
        .done(function(datos) {
...
function promedio(objetosInputs, valorMinimo, valorMaximo){
	...
	objetosInputs.each(function(i,element){
		if(element.value.length > 0){
			...
			if(parseFloat(element.value).toFixed(1)>= valorMinimo && element.value <= valorMaximo){
				console.log(parseFloat(element.value).toFixed(1));
