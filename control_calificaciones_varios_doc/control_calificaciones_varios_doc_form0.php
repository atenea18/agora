...
?>
 ...
 <?php
 if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_calificaciones_varios_doc']['embutida_pdf']))
 {
 ?>
...
?>
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>control_calificaciones_varios_doc/control_calificaciones_varios_doc_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
...
<?php
include_once("control_calificaciones_varios_doc_sajax_js.php");
?>
<script type="text/javascript">
...
include_once('control_calificaciones_varios_doc_jquery.php');
...
<?php
$str_iframe_body = ('F' == $_SESSION['sc_session'][$this->Ini->sc_page]['control_calificaciones_varios_doc']['run_iframe'] || 'R' == $_SESSION['sc_session'][$this->Ini->sc_page]['control_calificaciones_varios_doc']['run_iframe']) ? 'margin: 2px;' : '';
...
if ($GLOBALS["erro_incl"] == 1)
{
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['control_calificaciones_varios_doc']['opc_ant'] = "novo";
    $_SESSION['sc_session'][$this->Ini->sc_page]['control_calificaciones_varios_doc']['recarga'] = "novo";
}
if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['control_calificaciones_varios_doc']['recarga']))
{
    $opcao_botoes = $this->nmgp_opcao;
}
else
{
    $opcao_botoes = $_SESSION['sc_session'][$this->Ini->sc_page]['control_calificaciones_varios_doc']['recarga'];
}
?>
...
<?php
 include_once("control_calificaciones_varios_doc_js0.php");
?>
<script type="text/javascript"> 
...
<?php
$_SESSION['scriptcase']['error_span_title']['control_calificaciones_varios_doc'] = $this->Ini->Error_icon_span;
$_SESSION['scriptcase']['error_icon_title']['control_calificaciones_varios_doc'] = '' != $this->Ini->Err_ico_title ? $this->Ini->path_icones . '/' . $this->Ini->Err_ico_title : '';
?>
...
<?php
  if (!$this->Embutida_call && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_calificaciones_varios_doc']['mostra_cab']) || $_SESSION['sc_session'][$this->Ini->sc_page]['control_calificaciones_varios_doc']['mostra_cab'] != "N"))
  {
?>
<tr><td>
<style>
...
<?php
  ...
  {
       if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['control_calificaciones_varios_doc']['where_filter']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['control_calificaciones_varios_doc']['empty_filter'] = true;
       }
  }
  else
  {
...
<?php } else { $sc_hidden_no++; ?>
<?php  $this->docente_1 = explode(",", trim($this->docente)) ; ?>
...
else
{
   ...
   while (!empty($todo[$x])) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
          if ($cadaselect[1] == "@ ") {$cadaselect[1]= trim($cadaselect[1]); } ; 
          foreach ($this->docente_1 as $Dados)
          {
              if ($Dados === $cadaselect[1])
              {
                  $docente_look .= $cadaselect[0] . "&nbsp;";
                  break;
              }
          }
          $x++; 
...
else
{
   ...
   echo " <span id=\"idAjaxSelect_docente\"><select class=\"sc-js-input scFormObjectOdd css_docente_obj\" style=\"\" id=\"id_sc_field_docente\" name=\"docente_orig\" size=\"5\" multiple onDblClick=\"nm_add_sel('docente_orig', 'docente_dest', null);  \" alt=\"{type: 'select', enterTab: false}\">" ; 
   echo "\r" ; 
   while (!empty($todo[$x]) && !$nm_nao_carga) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
   ...
   echo "<div class='scBtnPassField' id='docente_all_right'>";
   echo 
        $sCondStyle = '';

        echo nmButtonOutput($this->arr_buttons, "bpassfld_rightall", "nm_add_all('docente_orig', 'docente_dest', null);    return false;", "nm_add_all('docente_orig', 'docente_dest', null);    return false;", "Bbpassfld_rightall", "", "", "" . $sCondStyle . "", "", "", "", $this->Ini->path_botoes, "", "", "", "", "");
;
   echo "</div>";
   ...
   echo 
        ...
        echo nmButtonOutput($this->arr_buttons, "bpassfld_right", "nm_add_sel('docente_orig', 'docente_dest', null);    return false;", "nm_add_sel('docente_orig', 'docente_dest', null);    return false;", "Bbpassfld_righ", "", "", "" . $sCondStyle . "", "", "", "", $this->Ini->path_botoes, "", "", "", "", "");
;
   echo "</div>";
   echo "<div class='scBtnPassField'>";
   echo 
        ...
        echo nmButtonOutput($this->arr_buttons, "bpassfld_left", "nm_del_sel('docente_dest', 'docente_orig', null);    return false;", "nm_del_sel('docente_dest', 'docente_orig', null);    return false;", "Bbpassfld_left", "", "", "" . $sCondStyle . "", "", "", "", $this->Ini->path_botoes, "", "", "", "", "");
;
   echo "</div>";
   echo "<div class='scBtnPassField' id='docente_all_left'>";
   echo 
        $sCondStyle = '';

        echo nmButtonOutput($this->arr_buttons, "bpassfld_leftall", "nm_del_all('docente_dest', 'docente_orig', null);    return false;", "nm_del_all('docente_dest', 'docente_orig', null);    return false;", "Bbpassfld_leftall", "", "", "" . $sCondStyle . "", "", "", "", $this->Ini->path_botoes, "", "", "", "", "");
;
   echo "</div>";
   echo "</td>";
   echo "<td>";
   echo " <select class=\"sc-js-input scFormObjectOdd css_docente_obj\" style=\"\" name=\"docente_dest\"  onBlur=\"scCssBlur(this);\"  onFocus=\"scCssFocus(this);\"  size=5 multiple onDblClick=\"nm_del_sel('docente_dest', 'docente_orig', null);  \" alt=\"{type: 'select', enterTab: false}\">";
   $x = 0 ; 
   while (!empty($todo[$x])) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
   ...
   echo " </select>" ; 
   echo " <input type=\"hidden\" name=\"docente\" value=\"\">" ; 
   echo "</td></tr></table>";
   echo " <script>document.F1.docente_dest.selectedIndex = -1;</script>" ; 
   echo "\r" ; 
   echo "</span>";
?> 
<?php  }?>
...
<script>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_calificaciones_varios_doc']['masterValue']))
{
?>
...
?>
<script>
 parent.scAjaxDetailStatus("control_calificaciones_varios_doc");
 parent.scAjaxDetailHeight("control_calificaciones_varios_doc", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
}
