...
class grid_codigos_dese_grup_asig_xls
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   var $Tit_doc;
   //---- 
   function grid_codigos_dese_grup_asig_xls()
   {
   }
   ...
   function inicializa_vars()
   {
      ...
      $this->Arquivo   .= ".xls";
      $this->Tit_doc    = "grid_codigos_dese_grup_asig.xls";
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_codigos_dese_grup_asig']['xls_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_codigos_dese_grup_asig']['xls_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_codigos_dese_grup_asig']['xls_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_codigos_dese_grup_asig']['xls_name']);
          $this->Xls_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
      ...
      { 
          ...
          if ($Cada_col == "rel_desemp_posicion_periodo" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
             if (!NM_is_utf8($SC_Label))
              {
                  $SC_Label = sc_convert_encoding($SC_Label, "UTF-8", $_SESSION['scriptcase']['charset']);
          ...
          if ($Cada_col == "desempeno_alto" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
             if (!NM_is_utf8($SC_Label))
              {
                  $SC_Label = sc_convert_encoding($SC_Label, "UTF-8", $_SESSION['scriptcase']['charset']);
          ...
          if ($Cada_col == "desempeno_basico" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
             if (!NM_is_utf8($SC_Label))
              {
                  $SC_Label = sc_convert_encoding($SC_Label, "UTF-8", $_SESSION['scriptcase']['charset']);
   ...
   function NM_export_desempeno_codigo()
   {
         ...
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->Xls_col) . $this->Xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
         if (is_numeric($this->desempeno_codigo))
         {
             $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->Xls_col) . $this->Xls_row)->getNumberFormat()->setFormatCode('#,##0');
         }
         $this->Nm_ActiveSheet->setCellValue($this->calc_cell($this->Xls_col) . $this->Xls_row, $this->desempeno_codigo);
         $this->Xls_col++;
   }
   ...
   function NM_export_rel_desemp_posicion_posicion()
   {
         ...
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->Xls_col) . $this->Xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
         $this->Nm_ActiveSheet->setCellValueExplicit($this->calc_cell($this->Xls_col) . $this->Xls_row, $this->rel_desemp_posicion_posicion, PHPExcel_Cell_DataType::TYPE_STRING);
         $this->Xls_col++;
   }
   //----- rel_desemp_posicion_periodo
   function NM_export_rel_desemp_posicion_periodo()
   {
         $this->rel_desemp_posicion_periodo = html_entity_decode($this->rel_desemp_posicion_periodo, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->rel_desemp_posicion_periodo = strip_tags($this->rel_desemp_posicion_periodo);
         if (!NM_is_utf8($this->rel_desemp_posicion_periodo))
         {
             $this->rel_desemp_posicion_periodo = sc_convert_encoding($this->rel_desemp_posicion_periodo, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->Xls_col) . $this->Xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
         $this->Nm_ActiveSheet->setCellValueExplicit($this->calc_cell($this->Xls_col) . $this->Xls_row, $this->rel_desemp_posicion_periodo, PHPExcel_Cell_DataType::TYPE_STRING);
         $this->Xls_col++;
   }
   //----- desempeno_superior
   function NM_export_desempeno_superior()
   {
         ...
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->Xls_col) . $this->Xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
         $this->Nm_ActiveSheet->setCellValueExplicit($this->calc_cell($this->Xls_col) . $this->Xls_row, $this->desempeno_superior, PHPExcel_Cell_DataType::TYPE_STRING);
         $this->Xls_col++;
   }
   //----- desempeno_alto
   function NM_export_desempeno_alto()
   {
         $this->desempeno_alto = html_entity_decode($this->desempeno_alto, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->desempeno_alto = strip_tags($this->desempeno_alto);
         if (!NM_is_utf8($this->desempeno_alto))
         {
             $this->desempeno_alto = sc_convert_encoding($this->desempeno_alto, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->Xls_col) . $this->Xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
         $this->Nm_ActiveSheet->setCellValueExplicit($this->calc_cell($this->Xls_col) . $this->Xls_row, $this->desempeno_alto, PHPExcel_Cell_DataType::TYPE_STRING);
         $this->Xls_col++;
   }
   //----- desempeno_basico
   function NM_export_desempeno_basico()
   {
         $this->desempeno_basico = html_entity_decode($this->desempeno_basico, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->desempeno_basico = strip_tags($this->desempeno_basico);
         if (!NM_is_utf8($this->desempeno_basico))
         {
             $this->desempeno_basico = sc_convert_encoding($this->desempeno_basico, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->Xls_col) . $this->Xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
         $this->Nm_ActiveSheet->setCellValueExplicit($this->calc_cell($this->Xls_col) . $this->Xls_row, $this->desempeno_basico, PHPExcel_Cell_DataType::TYPE_STRING);
         $this->Xls_col++;
   }
   ...
   function monta_html()
   {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_codigos_dese_grup_asig']['xls_file']);
      if (is_file($this->Xls_f))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_codigos_dese_grup_asig']['xls_file'] = $this->Xls_f;
      }
