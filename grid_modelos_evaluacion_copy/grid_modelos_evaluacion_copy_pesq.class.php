...
class grid_modelos_evaluacion_copy_pesq
{
   var $Db;
   var $Erro;
   var $Ini;
    ...
    * @access  public
    */
   function grid_modelos_evaluacion_copy_pesq()
   {
   }
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['path_libs_php'] = $this->Ini->path_lib_php;
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_modelos_evaluacion_copy_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_modelos_evaluacion_copy_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_modelos_evaluacion_copy_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == 'autocomp_id_modelo')
      {
          $id_modelo = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_id_modelo($id_modelo);
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_modelo')
      {
          $modelo = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_modelo($modelo);
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
          foreach ($nmgp_def_dados as $Ind => $Lista)
   ...
   function lookup_ajax_id_modelo($id_modelo)
   {
      $id_modelo = substr($this->Db->qstr($id_modelo), 1, -1);
            $id_modelo_look = substr($this->Db->qstr($id_modelo), 1, -1); 
      $nmgp_def_dados = array(); 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct id_modelo from " . $this->Ini->nm_tabela . " where  id_modelo like '%" . $id_modelo . "%'"; 
      }
      else
      {
          $nm_comando = "select distinct id_modelo from " . $this->Ini->nm_tabela . " where  id_modelo like '%" . $id_modelo . "%'"; 
   ...
   function lookup_ajax_modelo($modelo)
   {
      $modelo = substr($this->Db->qstr($modelo), 1, -1);
            $modelo_look = substr($this->Db->qstr($modelo), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct modelo from " . $this->Ini->nm_tabela . " where  modelo like '%" . $modelo . "%'"; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
   ...
   function monta_condicao($nome, $condicao, $campo, $campo2 = "", $nome_campo="", $tp_campo="")
   {
      ...
      else
      {
         ...
         $nome_sum = "modelos_evaluacion_copy.$nome";
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_srch_titl'] ?> - modelos_evaluacion_copy</TITLE>
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_modelos_evaluacion_copy/grid_modelos_evaluacion_copy_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="grid_modelos_evaluacion_copy_ajax_search.js"></script>
 <script type="text/javascript" src="grid_modelos_evaluacion_copy_ajax.js"></script>
 <script type="text/javascript">
...
function NM_hide_menu()
{
 ...
 function nm_save_form(pos)
 {
  ...
  var str_out = "";
  str_out += 'SC_id_modelo_cond#NMF#' + search_get_sel_txt('SC_id_modelo_cond') + '@NMF@';
  str_out += 'SC_id_modelo#NMF#' + search_get_text('SC_id_modelo') + '@NMF@';
  str_out += 'id_ac_id_modelo#NMF#' + search_get_text('id_ac_id_modelo') + '@NMF@';
  str_out += 'SC_id_modelo_input_2#NMF#' + search_get_text('SC_id_modelo_input_2') + '@NMF@';
  str_out += 'SC_modelo_cond#NMF#' + search_get_sel_txt('SC_modelo_cond') + '@NMF@';
  str_out += 'SC_modelo#NMF#' + search_get_text('SC_modelo') + '@NMF@';
  str_out += 'id_ac_modelo#NMF#' + search_get_text('id_ac_modelo') + '@NMF@';
   ...
   $("#id_ac_id_modelo").autocomplete({
     ...
     $.ajax({
       ...
       data: {
          ...
          nmgp_parms: "NM_ajax_opcao?#?autocomp_id_modelo",
          max_itens: "10",
          cod_desc: "N",
     ...
     select: function (event, ui) {
       $("#SC_id_modelo").val(ui.item.value);
       $(this).val(ui.item.label);
       event.preventDefault();
     },
     change: function (event, ui) {
       if (null == ui.item) {
          $("#SC_id_modelo").val( $(this).val() );
       }
     }
   });
   $("#id_ac_modelo").autocomplete({
     source: function (request, response) {
     $.ajax({
       url: "index.php",
       dataType: "json",
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_modelo",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
     ...
     select: function (event, ui) {
       $("#SC_modelo").val(ui.item.value);
       $(this).val(ui.item.label);
       event.preventDefault();
     },
     change: function (event, ui) {
       if (null == ui.item) {
          $("#SC_modelo").val( $(this).val() );
       }
     }
   });
...
<div style="width: 100%">
 ...
 <div style="height:37px; border-width:0px 0px 1px 0px;  border-style: dashed; border-color:#ddd; display: block">
 	<table style="width:100%; border-collapse:collapse; padding:0;">
    	<tr>
        	<td id="lin1_col1" class="scFilterHeaderFont"><span><?php echo $this->Ini->Nm_lang['lang_othr_srch_titl'] ?> - modelos_evaluacion_copy</span></td>
...
<?php
   ...
   {
      global 
             $id_modelo_cond, $id_modelo, $id_modelo_input_2, $id_modelo_autocomp,
             $modelo_cond, $modelo, $modelo_autocomp,
      ...
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $id_modelo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['id_modelo']; 
          $id_modelo_input_2 = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['id_modelo_input_2']; 
          $id_modelo_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['id_modelo_cond']; 
          $modelo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['modelo']; 
          $modelo_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['modelo_cond']; 
          $documento = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['documento']; 
          $documento_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['documento_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['NM_operador']; 
      } 
      if (!isset($id_modelo_cond) || empty($id_modelo_cond))
      {
         $id_modelo_cond = "gt";
      }
      if (!isset($modelo_cond) || empty($modelo_cond))
      {
         $modelo_cond = "qp";
      }
      ...
      $str_hide_id_modelo = (in_array($id_modelo_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_modelo = (in_array($modelo_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_documento = (in_array($documento_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;

      $str_display_id_modelo = ('bw' == $id_modelo_cond) ? $display_aberto : $display_fechado;

      if (!isset($id_modelo) || $id_modelo == "")
      {
          $id_modelo = "";
      }
      if (isset($id_modelo) && !empty($id_modelo))
      {
         $tmp_pos = strpos($id_modelo, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $id_modelo = substr($id_modelo, 0, $tmp_pos);
         }
      }
      ...
      {
         $tmp_pos = strpos($modelo, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $modelo = substr($modelo, 0, $tmp_pos);
         }
      }
...
<?php
   if (is_file("grid_modelos_evaluacion_copy_help.txt"))
   {
 ...
 <TR align="center">
  ...
  <TD width="100%" height="">
   ...
   <tr>
      ...
      <TD class="scFilterLabelOdd"><?php echo (isset($this->New_label['id_modelo'])) ? $this->New_label['id_modelo'] : "Id Modelo"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_id_modelo_cond" name="id_modelo_cond" onChange="nm_campos_between(document.getElementById('id_vis_id_modelo'), this, 'id_modelo')">
       <OPTION value="gt" <?php if ("gt" == $id_modelo_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_grtr'] ?></OPTION>
       <OPTION value="lt" <?php if ("lt" == $id_modelo_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_less'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $id_modelo_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="bw" <?php if ("bw" == $id_modelo_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_betw'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_id_modelo" <?php echo $str_hide_id_modelo?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['id_modelo'])) ? $this->New_label['id_modelo'] : "Id Modelo";
 $nmgp_tab_label .= "id_modelo?#?" . $SC_Label . "?@?";
 $date_sep_bw = "";
?>
<?php
      $id_modelo_look = substr($this->Db->qstr($id_modelo), 1, -1); 
   ...
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct id_modelo from " . $this->Ini->nm_tabela . " where id_modelo = $id_modelo_look"; 
      }
      else
      {
          $nm_comando = "select distinct id_modelo from " . $this->Ini->nm_tabela . " where id_modelo = $id_modelo_look"; 
      ...
      if (isset($nmgp_def_dados[0][$id_modelo]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$id_modelo];
      }
      else
      {
          $sAutocompValue = $id_modelo;
      }
?>
<INPUT  type="text" id="SC_id_modelo" name="id_modelo" value="<?php echo NM_encode_input($id_modelo) ?>" size=11 alt="{datatype: 'integer', maxLength: 11, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_id_modelo" name="id_modelo_autocomp" size="11" value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'integer', maxLength: 11, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}">
        </TD>
       </TR>
       <TR valign="top">
        <TD id="id_vis_id_modelo"  <?php echo $str_display_id_modelo; ?> class="scFilterFieldFontOdd">
         <?php echo $date_sep_bw ?>&nbsp;
         <BR>
         <INPUT type="text" id="SC_id_modelo_input_2" name="id_modelo_input_2" value="<?php echo NM_encode_input($id_modelo_input_2) ?>" size=11 alt="{datatype: 'integer', maxLength: 11, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" class="sc-js-input sc-js-input sc-js-input scFilterObjectOdd">
      ...
      <TD class="scFilterLabelEven"><?php echo (isset($this->New_label['modelo'])) ? $this->New_label['modelo'] : "Modelo"; ?></TD>
     <TD class="scFilterFieldEven"> 
      <SELECT class="scFilterObjectEven" id="SC_modelo_cond" name="modelo_cond" onChange="nm_campos_between(document.getElementById('id_vis_modelo'), this, 'modelo')">
       <OPTION value="qp" <?php if ("qp" == $modelo_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $modelo_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $modelo_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $modelo_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_modelo" <?php echo $str_hide_modelo?> valign="top">
        <TD class="scFilterFieldFontEven">
           <?php
 $SC_Label = (isset($this->New_label['modelo'])) ? $this->New_label['modelo'] : "Modelo";
 $nmgp_tab_label .= "modelo?#?" . $SC_Label . "?@?";
 $date_sep_bw = "";
?>
<?php
      $modelo_look = substr($this->Db->qstr($modelo), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct modelo from " . $this->Ini->nm_tabela . " where modelo = '$modelo_look'"; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
      ...
      if (isset($nmgp_def_dados[0][$modelo]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$modelo];
      }
      else
      {
          $sAutocompValue = $modelo;
      }
?>
<INPUT  type="text" id="SC_modelo" name="modelo" value="<?php echo NM_encode_input($modelo) ?>" size=50 alt="{datatype: 'text', maxLength: 150, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_modelo" name="modelo_autocomp" size="50" value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 50, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
...
?>
<INPUT  type="text" id="SC_documento" name="documento" value="<?php echo NM_encode_input($documento) ?>" size=50 alt="{datatype: 'text', maxLength: 150, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_modelos_evaluacion_copy']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_modelos_evaluacion_copy']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
?>
   ...
   <INPUT type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   nm_campos_between(document.getElementById('id_vis_id_modelo'), document.F1.id_modelo_cond, 'id_modelo');
   document.F1.id_modelo.value = "";
   document.F1.id_modelo_autocomp.value = "";
   document.F1.id_modelo_input_2.value = "";
   nm_campos_between(document.getElementById('id_vis_modelo'), document.F1.modelo_cond, 'modelo');
   document.F1.modelo.value = "";
   document.F1.modelo_autocomp.value = "";
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "agora_IV/grid_modelos_evaluacion_copy";
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function salva_filtro()
   {
      global $NM_filters_save, $nmgp_save_name, $nmgp_save_option, $script_case_init;
          ...
          $NM_patch .= "grid_modelos_evaluacion_copy/";
          if (!is_dir($NM_patch))
          {
              $NMdir = mkdir($NM_patch, 0755);
          }
   ...
   function recupera_filtro()
   {
      ...
      $tp_fields     = array();
      $tb_fields_esp = array();
      $tp_fields['SC_id_modelo_cond'] = 'cond';
      $tp_fields['SC_id_modelo'] = 'text_aut';
      $tp_fields['id_ac_id_modelo'] = 'text_aut';
      $tp_fields['SC_id_modelo_input_2'] = 'text';
      $tp_fields['SC_modelo_cond'] = 'cond';
      $tp_fields['SC_modelo'] = 'text_aut';
      $tp_fields['id_ac_modelo'] = 'text_aut';
   ...
   {
      global $id_modelo_cond, $id_modelo, $id_modelo_input_2, $id_modelo_autocomp,
             $modelo_cond, $modelo, $modelo_autocomp,
      ...
      if (!empty($id_modelo_autocomp) && empty($id_modelo))
      {
          $id_modelo = $id_modelo_autocomp;
      }
      if (!empty($modelo_autocomp) && empty($modelo))
      {
          $modelo = $modelo_autocomp;
      }
      if (!empty($documento_autocomp) && empty($documento))
      {
          $documento = $documento_autocomp;
      }
      $id_modelo_cond_salva = $id_modelo_cond; 
      if (!isset($id_modelo_input_2) || $id_modelo_input_2 == "")
      {
          $id_modelo_input_2 = $id_modelo;
      }
      $modelo_cond_salva = $modelo_cond; 
      if (!isset($modelo_input_2) || $modelo_input_2 == "")
      {
          $modelo_input_2 = $modelo;
      }
      ...
      if ($id_modelo_cond != "in")
      {
          nm_limpa_numero($id_modelo, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      else
      {
          $Nm_sc_valores = explode(",", $id_modelo);
          ...
          $id_modelo = implode(",", $Nm_sc_valores);
      }
      if ($id_modelo_cond != "in")
      {
          nm_limpa_numero($id_modelo_input_2, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['id_modelo'] = $id_modelo; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['id_modelo_input_2'] = $id_modelo_input_2; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['id_modelo_cond'] = $id_modelo_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['modelo'] = $modelo; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['modelo_cond'] = $modelo_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['documento'] = $documento; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['documento_cond'] = $documento_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($id_modelo_cond != "in" && $id_modelo_cond != "bw" && !empty($id_modelo) && !is_numeric($id_modelo))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : Id Modelo";
      }
      if ($id_modelo_cond == "bw" && ((!empty($id_modelo) && !is_numeric($id_modelo)) || (!empty($id_modelo_input_2) && !is_numeric($id_modelo_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : Id Modelo";
      }
      if (!empty($this->Campos_Mens_erro)) 
   ...
   { 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['id_modelo'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          ...
          $this->cmp_formatado['id_modelo'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['id_modelo'] = $id_modelo;
      }
      $Conteudo = $id_modelo_input_2;
      if (strtoupper($id_modelo_cond) != "II" && strtoupper($id_modelo_cond) != "QP" && strtoupper($id_modelo_cond) != "NP" && strtoupper($id_modelo_cond) != "IN") 
      { 
      ...
      $this->cmp_formatado['id_modelo_input_2'] = $Conteudo;
      $modelo_look = substr($this->Db->qstr($modelo), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct modelo from " . $this->Ini->nm_tabela . " where modelo = '$modelo_look'"; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['modelo'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['modelo'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['modelo'] = $modelo;
      }
      $documento_look = substr($this->Db->qstr($documento), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct documento from " . $this->Ini->nm_tabela . " where documento = '$documento_look'"; 
      ...
      if (isset($id_modelo) || $id_modelo_cond == "nu" || $id_modelo_cond == "nn" || $id_modelo_cond == "ep" || $id_modelo_cond == "ne")
      {
         $this->monta_condicao("id_modelo", $id_modelo_cond, $id_modelo, $id_modelo_input_2, "id_modelo");
      }

      //----- $modelo
      $this->Date_part = false;
      if (isset($modelo) || $modelo_cond == "nu" || $modelo_cond == "nn" || $modelo_cond == "ep" || $modelo_cond == "ne")
      {
         $this->monta_condicao("modelo", $modelo_cond, $modelo, "", "modelo");
      }
   ...
   function finaliza_resultado()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['where_pesq']         = $this->comando . $this->comando_fim;
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['cond_pesq'] .= $this->NM_operador;
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_modelos_evaluacion_copy']['where_pesq_ant'] = $this->comando . $this->comando_fim;
