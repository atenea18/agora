...
class ConsolidadoController{
	...
	public function getConsolidadoAction($db){	
		...
		$superacion = $_POST['superacion']=="1"?true:false;
		...
		foreach($num_periodos as $key => $_periodo){			
			...
			if($consolidado_obj->getAreasEvaluadas($grupo, $_periodo,$academicas,$reprobados) != false){
				...
				if($area=="0"){
					...
					$estudiantesPuestos[$contador] = $consolidado_obj->getEstudiantesPromedios($grupo, $_periodo, "0");
				...
				if($area=="1")
				{
					...
					$estudiantesPuestos[$contador] = $consolidado_obj->getEstudiantesPromediosAreas($grupo,$_periodo,"0");
					$puestoPromedio[$contador] = $this->puestosPromedios_obj->obtenerPromedios($estudiantesPuestos[$contador], $db);
		...
		if($cerrar == "true" && $area=="0" && $reprobados == "0"){			
		...
		if($superacion){
			...
			$array_estudiantes_asignaturas = $consolidado_obj->getInformeFinalSuperacion($grupo, $_periodo,$academicas);			
			$asignaturasEvaluadas = $consolidado_obj->getAsignaturasSuperacion($grupo, $_periodo,$academicas,$reprobados);
			
			header("Access-Control-Allow-Origin: *");
			
			$view = new View(
				'consolidado', 
				'consolidadoSuperacion', 
				[					
					'min_bajo' => $valoraciones[1]['minimo'],				 
					'min_basico' => $valoraciones[2]['minimo'],
					'cantidad_periodos' => $cantidad_periodos_evaluados,
					'cantidad_periodos_evaluados' => $cantidad_periodos_evaluados,
					'max_superior' => $valoraciones[3]['maximo'],
					'informe' => $informe,
					'asignaturasEvaluadas' => $asignaturasEvaluadas,
					'array_estudiantes_asignaturas' => $array_estudiantes_asignaturas,
					'array_listado_estudiantes_evaluados' => $asignaturasEvaluadas
				]);
