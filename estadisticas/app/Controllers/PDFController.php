...
class PDFController{
	...
	public function getConsolidadoAction(){
		...
		$isSuperacion = $_POST['superacion']=="1"?true:false;
		...
		*/
		foreach ($arrayGrupos as $grup) {
			...
			$superacionesPdf = $isSuperacion==true?$consolidado_obj->superacionesPdf($grup):false;
	...
	private function resolveDataFiltro($data=array()){
		...
		foreach ($arrayGrupos as $grup) {
			...
			if ($informe) {			
				...
				if($area=="0"){
					$tablaPuestos = $reprobadas_obj->getAsignaturasFinal($grup, $periodo, $academicas);
					$estudiantesRepro = $reprobadas_obj->getEstudianteAsignaturasFiltro($grup, $periodo, $academicas, $operador, $numero);

				}
				if($area=="1")
				{
					$tablaPuestos = $reprobadas_obj->getAreasFinal($grup, $periodo, $academicas);
					$estudiantesRepro = $reprobadas_obj->getEstudianteAreasFiltro($grup, $periodo, $academicas, $operador, $numero);
