...
class EstudiantesModel extends BD{
	...
	public function getListadoNovedadesGrado($id_grado, $id_novedad, $ano){
		$date_inicio = $ano.'-01-01';
		$date_fin = $ano.'-12-31';

		$this->query = "
		SELECT n.novedad, gru.nombre_grupo, nov.idstudents, st.primer_apellido, 
		st.segundo_apellido, st.primer_nombre, st.segundo_nombre 
		FROM novedades_x_estudiante_fecha as nov 
		INNER JOIN students st ON st.idstudents = nov.idstudents
		INNER JOIN novedades n ON n.id_novedad = nov.id_novedad
		INNER JOIN t_grados gra ON gra.grado = '{$id_grado}'
		INNER JOIN t_grupos gru ON gru.id_grado = gra.id_grado
		INNER JOIN t_estudiante_grupo tg ON tg.idstudent = st.idstudents AND tg.id_grupo= gru.id_grupo
		WHERE nov.id_novedad = '{$id_novedad}' AND nov.fecha >= '{$date_inicio}' and nov.fecha <= '{$date_fin}'		
		";
		$this->execute_single_query();
	...
	public function getListadoNovedadesGrupo($id_grupo, $id_novedad, $ano){
		$date_inicio = $ano.'-01-01';
		$date_fin = $ano.'-12-31';

		$this->query = "
		SELECT n.novedad, nov.idstudents, st.primer_apellido, st.segundo_apellido, st.primer_nombre, st.segundo_nombre 
		FROM novedades_x_estudiante_fecha as nov 
		INNER JOIN students st ON st.idstudents = nov.idstudents
		INNER JOIN novedades n on n.id_novedad = nov.id_novedad
		INNER JOIN t_estudiante_grupo tg ON tg.idstudent = st.idstudents AND tg.id_grupo = '{$id_grupo}'
		WHERE nov.id_novedad = '{$id_novedad}' AND nov.fecha >= '{$date_inicio}' and nov.fecha <= '{$date_fin}'
		";
		$this->execute_single_query();
