...
public function ConsolidadoTable(
    ...
    $is_repro=false,
    $informe,
    $array_puesto_promedio_acumulado,
    $superacionesPdf,
    $isSuperacion
)
{
    ...
    $cols_row = $isSuperacion==true?2:$cols_row;
    $informe = $isSuperacion==true? $isSuperacion: $informe;
        ...
        if($id_estudiante != $estudiante['id_estudiante'])
        {
            ...
            foreach ($periodos_evaluados as $key => $valuePeriodo)
            {
                ...
                if(!$informe){
                    ...
                    $this->Cell($w[4],4,utf8_decode(round($_promedio,2)),1,0,'C',$fill);
            ...
            if($is_acumulados || $informe){
                ...
                $this->Cell($w[0],4,"",0,0,'C');
                $this->Cell($w[1],4,"",0,0,'LR');
                $this->Cell(8,4,$array_puesto_promedio_acumulado[$id_estudiante]['puesto'],1,0,'C',true);
                $this->Cell(10,4,"ACUM.",1,0,'C',true);
                $this->Cell($w[4],4,round($promedio,2),1,0,'C',true);
                ...
                foreach ($asignaturasEvaluadas as $asignatura)
                {
                    ...
                    $supe = false;
                    if($isSuperacion){
                          foreach($superacionesPdf as $superacion){
                            if($superacion['id_estudiante']==$id_estudiante and $superacion['id_asignatura']==$asignatura['id_asignatura'] ){
                               $supe =  $superacion['nota'];
                            }
                        }  
                    }
                    ...
                    if($is_repro){
                        ...
                        $superacion = $supe != false?"/".$supe:"";
                        $promedio_acumulado_asig = $promedio_acumulado_asig==0?"":$promedio_acumulado_asig;
                        $this->Cell($fullpart,4,$promedio_acumulado_asig.$superacion,1,0,'C',true);
                    }
            ...
            if($is_acumulados || $informe)
            {
                ...
                foreach ($asignaturasEvaluadas as $asignatura)
                {
                    ...
                    if($informe){
                        if($supe){
                           $valoracion_requerida = $min_basico<=$supe?0:$valoracion_requerida; 
                        }
