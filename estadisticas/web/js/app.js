		...
		$(element).click(function(ev){
			...
			.done(function(datos) {
				...
				if(seccion=="EJK"){
					...
					$('#selectGradosN').change(getTablaEstadisticas);
					$('#selectLectivo').change(getTablaEstadisticas);
					$('#selectLectivoN').change(getTablaEstadisticas);
				...
				$('#selectGrados').change(function(ev){
					...
					.done(function(datos) {
						...
						$("#btnIF").unbind("click");
						$('#gruposCheckbox').find('input').unbind("click");
						$('#selectGrupos').unbind("change");
						$('#selectPeriodos').unbind("change");
						$('#selectOperador').unbind("change");
						$('#selectNumero').unbind("change");

						$('#selectGrupos').empty().append(datos);
						$('#btnIF').on('click',getTablaEstadisticas);
				...
				function getTablaEstadisticas(ev){
					...
					if(informe){
						if(controladorAccion == "/Reprobadas/getReprobadasFiltro/"){
							$('#btnIF').html('Reprobrar Filtrados');
							$('#btnIF').removeClass("btn-success");
							$('#btnIF').addClass("btn-danger");
						}else{
							$('#btnIF').html('Cerrar Informe Final');							
							$('#btnIF').removeClass("btn-danger");
							$('#btnIF').addClass("btn-success");
						}
					...
					$('#inputSuperacion').val(isSuperacion);
					...
					jornada+'&ano='+ano+'&per_acumulados='+periodos_acumulados+'&informe='+informe+
					'&cerrarInforme='+cerrarInforme+'&db='+db+'&superacion='+isSuperacion+'&idnovedad='+nov+'&idgrado='+idGradoSeleccionado;
					console.log(cerrarInforme);
							...
							$.ajax({
								...
								beforeSend: function(xhr){
									...
									$img.attr('src', "/estadisticas/web/images/preloader.gif");
