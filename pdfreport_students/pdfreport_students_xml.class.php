...
class pdfreport_students_xml
{
   ...
   //---- 
   function pdfreport_students_xml()
   {
   ...
   function inicializa_vars()
   {
      ...
      $this->Arquivo     .= ".xml";
      $this->Tit_doc      = "pdfreport_students.xml";
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_students']['xml_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_students']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_students']['xml_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_students']['xml_name']);
      }
      if (!$this->Grava_view)
      {
      ...
      while (!$rs->EOF)
      {
         $this->xml_registro = "<pdfreport_students";
...
$_SESSION['scriptcase']['pdfreport_students']['contr_erro'] = 'off'; 
   ...
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->ips))
         {
             $this->ips = sc_convert_encoding($this->ips, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " ips =\"" . $this->trata_dados($this->ips) . "\"";
   }
   //----- ars
   function NM_export_ars()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->ars))
         {
             $this->ars = sc_convert_encoding($this->ars, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " ars =\"" . $this->trata_dados($this->ars) . "\"";
   }
   //----- tipo_sangre
   ...
   function NM_export_victima_conflicto()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->victima_conflicto))
         {
             $this->victima_conflicto = sc_convert_encoding($this->victima_conflicto, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " victima_conflicto =\"" . $this->trata_dados($this->victima_conflicto) . "\"";
   }
   //----- estatus_vca
   function NM_export_estatus_vca()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->estatus_vca))
         {
             $this->estatus_vca = sc_convert_encoding($this->estatus_vca, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " estatus_vca =\"" . $this->trata_dados($this->estatus_vca) . "\"";
   }
   //----- depto_expulsor
   function NM_export_depto_expulsor()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->depto_expulsor))
         {
             $this->depto_expulsor = sc_convert_encoding($this->depto_expulsor, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " depto_expulsor =\"" . $this->trata_dados($this->depto_expulsor) . "\"";
   }
   //----- municipio_expulsor
   function NM_export_municipio_expulsor()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->municipio_expulsor))
         {
             $this->municipio_expulsor = sc_convert_encoding($this->municipio_expulsor, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " municipio_expulsor =\"" . $this->trata_dados($this->municipio_expulsor) . "\"";
   }
   //----- fecha_expulsion
   function NM_export_fecha_expulsion()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->fecha_expulsion))
         {
             $this->fecha_expulsion = sc_convert_encoding($this->fecha_expulsion, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " fecha_expulsion =\"" . $this->trata_dados($this->fecha_expulsion) . "\"";
   }
   //----- certificado
   function NM_export_certificado()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->certificado))
         {
             $this->certificado = sc_convert_encoding($this->certificado, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " certificado =\"" . $this->trata_dados($this->certificado) . "\"";
   }
   //----- numero_carne_sisben
   function NM_export_numero_carne_sisben()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->numero_carne_sisben))
         {
             $this->numero_carne_sisben = sc_convert_encoding($this->numero_carne_sisben, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " numero_carne_sisben =\"" . $this->trata_dados($this->numero_carne_sisben) . "\"";
   }
   //----- nivel_sisben
   ...
   function NM_export_fuente_recurso()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->fuente_recurso))
         {
             $this->fuente_recurso = sc_convert_encoding($this->fuente_recurso, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " fuente_recurso =\"" . $this->trata_dados($this->fuente_recurso) . "\"";
   }
   //----- opcion
   function NM_export_opcion()
   {
         ...
         $this->xml_registro .= " opcion =\"" . $this->trata_dados($this->opcion) . "\"";
   }
   //----- resguardo
   function NM_export_resguardo()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->resguardo))
         {
             $this->resguardo = sc_convert_encoding($this->resguardo, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " resguardo =\"" . $this->trata_dados($this->resguardo) . "\"";
   }
   //----- negritudes
   function NM_export_negritudes()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->negritudes))
         {
             $this->negritudes = sc_convert_encoding($this->negritudes, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " negritudes =\"" . $this->trata_dados($this->negritudes) . "\"";
   }
   //----- etnia
   ...
   function NM_export_discapacidades()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->discapacidades))
         {
             $this->discapacidades = sc_convert_encoding($this->discapacidades, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " discapacidades =\"" . $this->trata_dados($this->discapacidades) . "\"";
   }
   //----- capacidades
   function NM_export_capacidades()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->capacidades))
         {
             $this->capacidades = sc_convert_encoding($this->capacidades, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " capacidades =\"" . $this->trata_dados($this->capacidades) . "\"";
   }
   //----- postalcode
   function NM_export_postalcode()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->postalcode))
         {
             $this->postalcode = sc_convert_encoding($this->postalcode, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " postalcode =\"" . $this->trata_dados($this->postalcode) . "\"";
   }
   //----- email
   ...
   function NM_export_peso()
   {
         ...
         $this->xml_registro .= " peso =\"" . $this->trata_dados($this->peso) . "\"";
   }
   //----- talla
   function NM_export_talla()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->talla))
         {
             $this->talla = sc_convert_encoding($this->talla, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " talla =\"" . $this->trata_dados($this->talla) . "\"";
   }
   //----- cobertura
   function NM_export_cobertura()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->cobertura))
         {
             $this->cobertura = sc_convert_encoding($this->cobertura, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " cobertura =\"" . $this->trata_dados($this->cobertura) . "\"";
   }
   //----- vive_con
   function NM_export_vive_con()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->vive_con))
         {
             $this->vive_con = sc_convert_encoding($this->vive_con, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " vive_con =\"" . $this->trata_dados($this->vive_con) . "\"";
   }
   //----- subsidio
   ...
   function monta_html()
   {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_students']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_students']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
