...
class GradeBook2PDF extends FPDF
{
	...
	private function subHeader()
	{
		if($this->gradeBook['config'][0]['periodIF']):
			...
			$this->Cell(140, $this->_h_c, $this->gradeBook['tittle'].utf8_decode(' PERIODO '.$this->gradeBook["current_period"].' - AÑO LECTIVO 2017'),1,0,'L'); 
		endif;
	...
	*/
	...
	{	
		...
		$pahtImage = "http://2017.ateneasas.com/boletin/Public/img/";
		$height = 0;
		$note = 0;
		$valoration = '';
	...
	*/
	...
	public function reprobateByAsignature()
	{
		...
		return ($hit >= $this->minAsignature) ? array('text' => '* NO APROBÓ', 'state'=>false, 'hit'=> $hit) : array('text' => '* SI APROBÓ', 'state' => true, 'hit'=> $hit);
	}
	...
	private function showSomeReprobation()
	{
		$data = array();
	...
	private function showCleanReprobation()
	{
		$data = array();
