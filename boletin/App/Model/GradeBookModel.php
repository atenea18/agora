...
class GradeBookModel extends DB
{
	...
	*/
	public function init()
	{
		...
		if($this->request['period'] == 'if'):
			$this->current_period = count($this->periods);
			$this->isIF = true;
		else: 
	...
	*
	*/
	public function resolveReportFinal($id_student, $id_group, $periods)
	{	

		return ['id_student'=>$id_student, 'id_group'=>$id_group,'periods'=>$periods];
	}
		...
		foreach ($estudiante as $key => $value) {
			...
			if($value['pgg']>$pggAux){
				$estudiantePgg = array(
					...
					'pgg' => $value['pgg'],
					'average' => $this->determineRound($value['promedio'], 2),
					'period'	=> $period
				);
				$pggAux = $value['pgg'];
			...
			foreach($areas['data'] as $key => $area):
				...
				array_push(
					...
					array(
						...
						'nota'			=>	$this->determineRound($area['Valoracion'], 2),
	...
	*/
	...
	{
		$number = $this->roundNumber($value, $roundNumber);
		
		if(!$this->_config[0]['decimals'])
			...
			if($asignature['id_asignatura'] == $performance['id_asignatura'] && $performance['periodo'] == $period && strstr($performance['posicion'], $prefix)):
				...
				// if( (strstr($performance['posicion'], $prefix)) || () ):
					...
					array_push($response, 
						...
						array(
							'codigo'	=>	$performance['codigo'],
							'note'	=>	$this->determineRound($per_note,2),
							'valoration'	=>	$valoration,
	...
	*/
	...
	{	
		...
		if($resp['state']):
			...
			$response['recovery_note'] = $this->determineRound( 
				$resp['data'][0]['nota'], 2
			);

			$response['old_note'] = $this->determineRound(
	...
	*/
	...
	{
		$data = $this->_evaluation->getFinalReportList($id_group, $year)['data'];

		$response = array();

		foreach($data as $key => $value):
			$note = $this->resolveNoteFinalReport(
					$value['definitiva'], $value['id_estudiante'], $value['id_grupo'], $value['id_asignatura']);
			array_push(
				$response, 
				array(
					'id_student' => $value['id_estudiante'],
					'id_group'	=> $value['id_grupo'],
					'id_area'	=> $value['id_area'],
					'int_hor'	=>	($value['int_horaria'] > 0) ? $value['int_horaria'] : '',
					'promedio'	=>	$this->determineRound($value['promedio'], 2),
					'puesto'	=>	$value['puesto'],
					'id_asignature'	=>	$value['id_asignatura'],
					'name' => $value['asignatura'],
					'type' => $value['tipo_asig'],
					'year'	=> $value['ano_lectivo'],
					'nota'	=> $this->determineRound($note, 2),
					'valoration' => $this->getValoration( $value['definitiva'] ),
				)
			);
		endforeach;
	...
	public function getFinalReportListByStudent($student_id, $id_group, $grade_id, $year)
	{
		$data = $this->_evaluation->getFinalReportListByStudents($student_id, $id_group, $grade_id, $year)['data'];
		...
		foreach($data as $key => $value):
			...
			$note = $this->determineRound($note, 2);
			array_push(
				$response, 
				array(
					'id_student' => $value['id_estudiante'],
	...
	private function resolveNoteFinalReport($note = 0, $student_id, $group_id, $asignature_id)
	{
		$recovery = $this->_evaluation->getRecoveryFinalReport(
			$student_id, $group_id, $asignature_id
		);

		if($recovery['state']):
			$note = ($recovery['data'][0]['nota'] > $note) ? $recovery['data'][0]['nota'] : $note ;
		endif;

		return $note;
	}

	public function getAverageAreaFinalReport($id_group, $year)
	{
		$data = $this->_evaluation->getAverageAreasFinalReport($id_group, $year)['data'];
		...
		foreach($data as $key => $value):
			array_push(
				...
				array(
					...
					'type' => $value['tipo'],
					'year'	=> $value['ano_lectivo'],
					'note'	=> $this->determineRound($value['definitiva'], 2),
					'valoration' => $this->getValoration( $this->determineRound($value['definitiva'], 2) /*$value['definitiva']*/),
				)
			);
		endforeach;
