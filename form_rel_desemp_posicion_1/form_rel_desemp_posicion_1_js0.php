<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="id_grado_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['id_grado_']); ?>">
<input type="hidden" name="id_grupo_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['id_grupo_']); ?>">
<input type="hidden" name="id_asign_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['id_asign_']); ?>">
<input type="hidden" name="periodo_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['periodo_']); ?>">
<input type="hidden" name="posicion_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['posicion_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_rel_desemp_posicion_1"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{ 
    ...
    Nm_Proc_Atualiz = true;
    do_ajax_form_rel_desemp_posicion_1_submit_form(y, z); 
} 
<?php
}
?>
...
var nm_num_campo = 1;
function lookup_cod_desemp_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.cod_desemp_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_rel_desemp_posicion_1')?>/form_rel_desemp_posicion_1.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&script_case_session=<?php echo session_id() ?>&nmgp_opcao=lookup", recebe_cod_desemp_, "cod_desemp_", dados, false);
}
function recebe_cod_desemp_(string)
{
   document.getElementById('id_lookup_cod_desemp_' + nm_num_campo).innerHTML = string;
}
<?php 
