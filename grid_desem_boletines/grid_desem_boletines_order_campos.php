<?php
   ...
   if (!function_exists("NM_is_utf8"))
   {
    ...
    $Ord_Cmp = new grid_desem_boletines_Ord_cmp(); 
    $Ord_Cmp->Ord_cmp_init();
   
class grid_desem_boletines_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $tab_ger_campos, $tab_def_campos, $tab_converte, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   $tab_labels["desem_asig1"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_desem_boletines']['labels']["desem_asig1"])) ? $_SESSION['sc_session'][$sc_init]['grid_desem_boletines']['labels']["desem_asig1"] : "desem_asig1";
   $tab_ger_campos['desem_asig1'] = "none";
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_desem_boletines']['field_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['grid_desem_boletines']['field_display']))
   {
       foreach ($_SESSION['scriptcase']['sc_apl_conf']['grid_desem_boletines']['field_display'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_desem_boletines']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_desem_boletines']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_desem_boletines']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_desem_boletines']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_desem_boletines']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_desem_boletines']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_desem_boletines']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_desem_boletines']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       {
           $_SESSION['sc_session'][$sc_init]['grid_desem_boletines']['ordem_select'][$campo] = $ordem;
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   $.ajax({
    type: "POST",
    url: "grid_desem_boletines_order_campos.php",
    data: {
...
<?php
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if (!isset($_SESSION['sc_session'][$sc_init]['grid_desem_boletines']['ordem_select'][$tab_def_campos[$NM_cada_field]]))
           {
?>
...
<?php
   foreach ($_SESSION['sc_session'][$sc_init]['grid_desem_boletines']['ordem_select'] as $NM_cada_field => $NM_cada_opc)
   {
